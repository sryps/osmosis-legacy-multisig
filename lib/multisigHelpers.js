...
const createMultisigFromCompressedSecp256k1Pubkeys = async (
  ...
  try {
    ...
    //check already exist in db before request
    const _res = await axios.get("/api/multisig", {
      params : {address_bech32: multisigAddress}
    });

    if (_res.data.getMultisig) {
      console.log(_res.data);
      console.log("don't create new multisig")
 ...
 * @param client A connected stargate osmosis client
